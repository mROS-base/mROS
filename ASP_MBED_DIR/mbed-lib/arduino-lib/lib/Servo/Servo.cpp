...
void resetPinModeServo(int pin)
{
...
uint8_t Servo::attach(int pin, int min, int max)
{
  if(this->servoIndex < MAX_SERVOS ) {
    ...
    if (isHardwarePWMPin(pin)) {
      ...
      int term = HardwarePWMFreq * (long long)servos[this->servoIndex].ticks / (1000 * 1000);
      setPinModeHardwarePWM(pin, period, term, 0);
    } else {
      int period = SoftwarePWMFreq * (long long)REFRESH_INTERVAL / (1000 * 1000);
